...
function MyProjects() {
  return (
    <div className="bg-black text-white px-4 sm:px-6 lg:px-12 py-12 lg:py-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {projects.map((project, index) => (
          <div
            key={index}
            className="relative flex-shrink-0 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300"
          >
            <img
              src={project.imageUrl}
              alt={project.title}
              className="w-full h-52 object-cover transition-transform duration-300 transform hover:scale-105"
            />
      ...
      <div className="text-center mt-10">
        <button
          className="inline-flex items-center justify-center px-6 py-3 bg-[#0A0D2D] text-white rounded-full border border-gray-50 hover:bg-gray-800 hover:text-gray-100 transition duration-300 ease-in-out"
          type="submit"
        >
          View More
        </button>
      ...
      <div className="text-center mt-16">
        <h2 className="text-lg font-bold mb-4">All my projects are published on GitHub</h2>
        <p className="mb-6">Click the button below to view my GitHub profile and see more of my work.</p>
        <a
          href="https://github.com/EesaraMegasooriya"
          target="_blank"
          rel="noopener noreferrer"
          className="flex justify-center"
        >
          <button className="flex items-center px-6 py-3 bg-gray-800 text-white rounded-lg hover:bg-black transition duration-300">
            <svg className="w-5 h-5 mr-2 fill-current" viewBox="0 0 438.549 438.549">
              <path
